...
- 最终 prompt assembly 阶段，本模式会移除 `harness:identity`，并把 `deployment:persona` 替换为动态身份锚定，确保交给模型的第一句话是软件工程行为触发器：
  ...
  这里的 `{{model}}` 由 DSH 在渲染时填充，模型路由变化时，当前身份也会随之更新；Standard 本体不被修改。
